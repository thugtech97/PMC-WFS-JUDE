...
class TransactionsController extends Controller
{
    ...
    {        
        ...
        if(auth::user()->designation == 'MANAGER') {
            if( auth()->user()->username == 'jccadiao' ) {
                ...
                })->whereIn('status', ['PENDING','IN-PROGRESS','HOLD'])->get();
            } else {
                $transactions = $transactions->whereIn('status', ['PENDING', 'HOLD'])->get();
            }
        } else {
            $transactions = $transactions->whereIn('status', ['IN-PROGRESS', 'HOLD'])->get();
        }
    ...
    {        
        ...
        return view('transaction.details',compact('data','lastapprover','approverstatus','currentuser','previous','next','curr_seq','last_app_seq'));
    }
