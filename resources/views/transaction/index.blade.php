...
@section('pagecss')
    ...
    <style type="text/css">
        ...
        .table-container-class #data-table_wrapper .dt-buttons { position: absolute; left: 12%; top: 28px; }
...
@section('content')
                ...
                <div class="col-lg-9 d-flex align-items-center">
                    ...
                    <div class="page-header-title">
                        <div class="title-card-class wfs-bg">
                            ...
                            <div class="d-inline">
                                <h5 class="wfs-text-default"><b>Transactions</b></h5>
                                <span class="wfs-text-default" style="opacity: .75;">Below are the list of pending and on hold transactions.</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="wfs-stats-cards ml-4">
                        <div class="d-flex flex-row align-items-center">
                            <div class="title-card-class wfs-bg mr-2">
                                <span class="svg-class">
                                ...
                                <div class="d-inline">
                                    <h5 class="wfs-text-default mb-0"><b style="font-weight: 800">{{ $pending }}</b></h5>
                                    <span class="wfs-text-default" style="opacity: .75;">Pending</span>
                                </div>
                            </div>
                            <div class="title-card-class wfs-bg mr-2">
                                <span class="svg-class">
                                ...
                                <div class="d-inline">
                                    <h5 class="wfs-text-default mb-0"><b style="font-weight: 800">{{ $upcoming }}</b></h5>
                                    <span class="wfs-text-default" style="opacity: .75;">Upcoming</span>
                                </div>
                            </div>
                            <div class="title-card-class wfs-bg mr-2">
                                <span class="svg-class">
                                ...
                                <div class="d-inline">
                                    <h5 class="wfs-text-default mb-0"><b style="font-weight: 800">{{ $inprogress }}</b></h5>
                                    <span class="wfs-text-default" style="opacity: .75;">In Progress</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <tbody>
                        @foreach($transactions as $t)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('transaction.details',$t->id) }}" class="scale-1" title="View Details">
                                        ...
                                        <span style="font-weight: bold; font-size: 11px">
                                             {{ $t->details }} @if(empty($t->locsite)) (Agusan) @elseif(!empty($t->locsite)) (Davao) @endif
                                        </span>
                                    </a>
                                </td>
                                <td><b>{{ $t->requestor }}</b><br><small>{{ $t->department }}</small></td>
                                <td>{{ $t->created_at != NULL ? $t->created_at->translatedFormat('F j, Y @g:ia') : ''}}</td> 
                                
                                <td>{{ $t->name }}</td> <!-- MISSING FOR VBS REQUESTS -->
                                <td class="amount-td"><b>{{ $t->currency }} {{ number_format($t->totalamount,2) }}</b> <br> <small>@if($t->totalamount < $t->converted_amount) (PHP {{ number_format($t->converted_amount,2) }}) @endif</small></td>
                                ...
                                <td style="max-width: 260px !important; overflow: hidden; text-overflow: ellipsis;">{{ $t->purpose }}</td>
                                <td hidden>
    ...
    <!-- Bulk Cancel Modal -->
    <div class="modal fade" id="batchCancelModal" tabindex="-1" role="dialog" aria-labelledby="batchCancelModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title d-flex align-items-center" id="batchCancelModalLabel">
                        <b>
                            <span>
                                <svg style="transform: translate(0px, -3px);" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                <div class="modal-body">
                    <form action="{{ route('transaction.batchsubmit') }}" method="POST" id="canceldata"> 
                        ...
                        <div class="form-group">
                            <textarea class="form-control" name="cancel_reason" id="cancel_reason" rows="6" placeholder="Type cancel reason here.."></textarea>
                        </div>
                        <input type="hidden" class="selected_data_cancel" name="selected_data" id="selected_data" class="" readonly>        
                        <input type="hidden" class="selected_action_cancel" name="selected_action" id="selected_action" readonly>    
                </div>
   ...
   <!--  <script type="text/javascript">
    ...
    $("#data-table").dataTable({
          // Internationalisation. For more info refer to http://datatables.net/manual/i18n
            ...
            dom: 'lBfrtip',
            buttons: [
                { extend: 'excel', className: 'wfs-bg', text: `
                                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="28" height="18" fill="currentColor" viewBox="0 0 24 24">
    ...
    // jeff said: Let's add a status and transaction type filter unto the data table.
...
<script>
    ...
    // jeff said: Open the batch cancel modal
