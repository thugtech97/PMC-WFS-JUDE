...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header"><h3>Update Template</h3></div>
            <div class="card-body">
                <form autocomplete="off" method="post" action="{{ route('templates.update',$template->id) }}">
                @csrf
                @method('put')
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" id="exampleInputEmail3" placeholder="Template name" value="{{ $template->name }}">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>Approvers</label>
                                <select class="form-control" id="selected_approver">
                                    <option selected disabled>Select Approver</option>
                                    @foreach($approvers as $a)
                                    <option value="{{$a->id}}|{{$a->name}}|{{$a->designation}}|{{$a->condition}}|{{$a->department}}|{{$a->section}}|{{$a->is_dynamic}}|{{$a->division}}">{{$a->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row clearfix" id="approvers">
                        @foreach($template->approvers as $approver)
                        <div class="col-xl-3 col-md-6" id="row{{$approver->user->id}}">
                            <div class="card ticket-card">
                                <div class="card-body">
                                    <button class="btn mb-15 btn-secondary btn_remove" id="{{$approver->user->id}}">Remove</button>
                                    <button type="button" onclick="addAlternateApprover('{{$approver->approver_id}}')" class="btn btn-success btn-icon float-md-right"><i class="ik ik-plus"></i></button>
                                    <input type="hidden" name="approver_id[]" value="{{$approver->approver_id}}"/>
                                    <input type="hidden" name="al_approver_id[]" id="al_id{{$approver->approver_id}}"/>
                                    <div class="text-center">
                                        <img src="{{ asset('assets/img/users/user.png') }}" style="height: 70px;" alt="">
                                        <br>
                                        <p class="mb-0 d-inline-block">{{$approver->user->name}}</p>
                                        <br>
                                        <p class="mb-5 d-inline-block text-uppercase"><small>{{$approver->user->designation}}</small></p>
                                        <br>
                                        Alternate Approver : <p class="mb-5 d-inline-block text-uppercase" id="alternate_approver{{$approver->approver_id}}"><small>{{ \App\User::alternate_approver($approver->alternate_approver_id) }}</small></p>
                                    </div>
                                </div>
                            </div>
                        </div>
